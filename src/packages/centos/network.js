...
export default {
  ...
  description: 'sysctl stack, EPEL setup, protocol blacklist, dnf-automatic',
  category: 'Network',
  defaultEnabled: true,

  options: [
    { id: 'enableEPEL',       type: 'confirm', label: 'Enable EPEL repo (needed for fail2ban, lynis, etc.)?', default: true },
    { id: 'enableCRB',        type: 'confirm', label: 'Enable CodeReady Builder / PowerTools repo?',          default: true },
  ...
  generate({ options }) {
    const epel      = options.enableEPEL      ?? true;
    const crb       = options.enableCRB       ?? true;
...
${crb ? `
# Enable CodeReady Builder (CRB) / PowerTools repo
dnf config-manager --set-enabled crb 2>/dev/null || \
dnf config-manager --set-enabled powertools 2>/dev/null || \
dnf config-manager --set-enabled CodeReady-Builder 2>/dev/null || true
` : ''}
