...
export default {
  ...
  description: 'sysctl stack, protocol blacklist, dnf-automatic, subscription-manager check',
  category: 'Network',
  defaultEnabled: true,
  ...
  generate({ options }) {
    ...
    return `
# ── Network Hardening (RHEL) ──────────────────────────────────────────

# Check RHEL subscription (warn if not registered — some packages need it)
if command -v subscription-manager &>/dev/null; then
  STATUS=$(subscription-manager status 2>/dev/null | grep 'Overall Status' | awk -F: '{print $2}' | xargs)
  if [ "$STATUS" != "Current" ]; then
    echo "[!] RHEL subscription not current — some repos may be unavailable."
    echo "[!] Run: subscription-manager register"
  else
    echo "[+] RHEL subscription: $STATUS"
    # Enable CodeReady Builder (needed for some security packages)
...
grub2-mkconfig -o /boot/grub2/grub.cfg 2>/dev/null || grub2-mkconfig -o /boot/efi/EFI/redhat/grub.cfg 2>/dev/null || true
` : ''}
`;
  },
