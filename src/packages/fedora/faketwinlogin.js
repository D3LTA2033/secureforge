export default {
  ...
  description: 'Two passwords — real OS vs decoy session via PAM exec hook',
  category: 'Deception',
  ...
  generate({ options }) {
    ...
    return `
# ── Fake Twin Login / Duress Login (Fedora) ──────────────────────────
...
# Lock decoy user's real password (only accessible via duress hook)
...
# ── 3. PAM exec duress check script ──────────────────────────────────
...
if [ "$STORED" = "$ENTERED" ]; then
  ...
  ${logAccess ? `logger -t secureforge-duress "DURESS LOGIN: user=$PAM_USER src=$SRC_IP tty=\${PAM_TTY:-unknown}"` : ''}
...
# ── 4. Inject into PAM (system-auth via authselect custom profile) ────
# On Fedora, we inject into the authselect-managed system-auth
...
# Allow SELinux to exec the duress script
restorecon -Rv /usr/local/bin/sf-duress-check 2>/dev/null || true
chcon -t bin_t /usr/local/bin/sf-duress-check 2>/dev/null || true

echo "[+] Fake Twin Login configured."
echo "[!] Duress password → '${decoyUser}' decoy session."
echo "[!] Real password   → your real account."
`;
  },
