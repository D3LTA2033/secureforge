...
export default {
  ...
  options: [
    ...
    { id: 'totp',                type: 'confirm', label: 'Enable TOTP 2FA (google-authenticator-libpam)?', default: false },
...
# SELinux: allow custom SSH port if changed
${port !== 22 ? `semanage port -a -t ssh_port_t -p tcp ${port} 2>/dev/null || semanage port -m -t ssh_port_t -p tcp ${port}` : ''}
...
`;
  ...
  manifests() {
    return {
      ...
      packages_installed: ['openssh-server'],
    };
  },
};
